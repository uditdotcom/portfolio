...
const roles = introductionData.roles;
let roleIndex = 0;
let charIndex = 0;

const introElement = document.getElementById('introText');
const typingSpeed = 100; // milliseconds per character
const pauseAfterFullWord = 1500; // pause before typing next word

const templateNameApresentation = (dynamicText) => {
    return `
    <div>Hi.. I'm <span class="gradient">Udit Gupta</span></div>
    <div><span class="gradient">${dynamicText}</span></div>
`;
};
...
function typeRole() {
    ...
    const partialText = currentRole.substring(0, charIndex + 1);

    introElement.innerHTML = templateNameApresentation(partialText);
